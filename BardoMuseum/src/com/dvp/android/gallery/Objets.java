...
public class Objets extends Activity implements Runnable {
	ListView list;
	ProgressDialog pd;
...
public void execute_search() {
	pd = ProgressDialog.show(Objets.this, "", "Chargemnet...", true,false);

	Thread thread = new Thread(Objets.this);
	thread.start();
...
public void get_liste_item_service(){
	...
	String url = "http://10.0.2.2/www/Bardo/objets.php";
    ...
    ListObjects response = null;
    try{
     response= gson.fromJson(reader, ListObjects.class);
    }
    catch (Exception e) {
...
public void get_list_item(){
	...
	{
		values_array =  new ArrayList<HashMap<String, String>>();
		ObjetAdapter adapter = new ObjetAdapter(this, R.layout.affichageitem, results);
